 ...
 *
 */
public class TernarySearchTrie<A> {

	/**
	 *  An inner class of Ternary Search Trie that represents a node in the trie.
	 */
	protected final class TSTNode<B> {
		...
		protected TSTNode<B>[] relatives = new TSTNode[4];
		 ...
		 */
		protected TSTNode(char splitchar, TSTNode<B> parent) {
	 ...
	 */
	private void deleteNode(TSTNode<A> nodeToDelete) {
	 ...
	 */
	private TSTNode<A> deleteNodeRecursion(TSTNode<A> currentNode) {
		...
		TSTNode<A> currentParent = currentNode.relatives[TSTNode.PARENT];
	...
	public A get(String key) {
		TSTNode<A> node = getNode(StringUtils.toLowerCase(key.trim(), false));
	 ...
	 */
	public TSTNode<A> getNode(String key) {
	 ...
	 */
	protected TSTNode<A> getNode(String key2, TSTNode<A> startNode) {
		String key = StringUtils.toLowerCase(key2.trim(), false);
		if (key == null || startNode == null || key.length() == 0) {
			return null;
		}
		TSTNode<A> currentNode = startNode;
		int charIndex = 0;
		while (true) {
			if (currentNode == null) {
				return null;
	 ...
	 */
	protected TSTNode<A> getOrCreateNode(String key)
		...
		if (rootNode == null) {
			rootNode = new TSTNode<A>(key.charAt(0), null);
		}
		TSTNode<A> currentNode = rootNode;
		int charIndex = 0;
		while (true) {
			int charComp =
				compareCharsAlphabetically(
			...
			if (charComp == 0) {
				...
				if (currentNode.relatives[TSTNode.EQKID] == null) {
					currentNode.relatives[TSTNode.EQKID] =
						new TSTNode<A>(key.charAt(charIndex), currentNode);
				}
				currentNode = currentNode.relatives[TSTNode.EQKID];
			} else if (charComp < 0) {
	...
	public List<String> matchPrefix(String prefix, int numReturnValues) {
		...
		TSTNode<A> startNode = getNode(prefix);
		if (startNode == null) {
	 ...
	 */
	protected int numDataNodes(TSTNode<A> startingNode) {
	 ...
	 *@return               The total number of nodes in the subtrie.
	 */
	protected int numNodes(TSTNode<A> startingNode) {
	...
	public void put(String key, A value) {
		getOrCreateNode(StringUtils.toLowerCase(key.trim(), false)).data = value;
	}
	 ...
	 *@return                  A <code>List</code> with the results.
	 */
	protected List<String> sortKeys(TSTNode<A> startNode, int numReturnValues) {
		return sortKeysRecursion(
			startNode,
			((numReturnValues < 0) ? -1 : numReturnValues),
			new Vector<String>());
